...
class Okru(private val client: OkHttpClient) {
    ...
    fun extract(url: String, prefix: String = "", fixQualities: Boolean = true): List<Video> {
        val document = runCatching { client.newCall(GET(url)).execute().asJsoup()}.getOrNull() ?: return emptyList<Video>()
