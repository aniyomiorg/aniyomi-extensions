...
object AYFilters {
    ...
    class LanguageFilter : QueryPartFilter("Idioma", AYFiltersData.languages)
    class InitialLetterFilter : QueryPartFilter("Primeira letra", AYFiltersData.initialLetter)

    class EpisodeFilter : AnimeFilter.Text("Episódios")
    class EpisodeFilterMode : QueryPartFilter("Modo de filtro", AYFiltersData.episodeFilterMode)
    class SortFilter : AnimeFilter.Sort(
        "Ordenar",
        AYFiltersData.orders.map { it.first }.toTypedArray(),
        Selection(0, true)
    )

    class GenresFilter : TriStateFilterList(
        "Gêneros",
        AYFiltersData.genres.map { TriStateVal(it) }
    )
    ...
    internal fun getSearchParameters(filters: AnimeFilterList): FilterSearchParams {
        ...
        filters.getFirst<SortFilter>().state?.let {
            val order = AYFiltersData.orders[it.index].second
